...
export default function PrivacyPage() {
  return (
    ...
    >
      ...
      <section>
        <h2>保存と利用</h2>
        <p>入力内容と認証情報は、依頼された変換または下書き作成のためだけに利用します。サーバー側のデータベース、ファイル、Cookie、アプリケーションログへは保存しません。Figma Tokenは標準では同じタブのセッションストレージだけに保持します。「このブラウザに保存する」を選んだ場合に限り、利用中のブラウザのローカルストレージへ保存します。どちらも入力欄の「消去」で削除できます。共有端末ではブラウザ保存を使用しないでください。</p>
      </section>
      <section>
        <h2>ホスティングと外部サービス</h2>
      ...
      <p className="legal-page__date">最終更新：2026年7月23日</p>
    </LegalLayout>
  );
}
