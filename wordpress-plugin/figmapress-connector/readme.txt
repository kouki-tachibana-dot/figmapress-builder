...
= 0.19.11 =
* 元ページを変更しない、検証コピー一式の専用作成処理を追加しました。
* 再試行時の重複・手修正の上書き・公開メニュー変更を防ぎます。
* 検証メニューには実コピーIDの下書きプレビューURLを使用します。

= 0.19.10 =
* ページ・タイトル・メニューを変更しない、権限付きのページ対応表取得APIを追加します。
* 重複、下書き以外、編集権限不足、識別子の不一致を明示し、推測のページIDを返しません。
* ペアリング接続・共有サーバーの安全接続にも読み取り専用の経路を追加します。

= 0.19.9 =
* スマホの白いメニューパネルへヘッダーの白文字を継承せず、通常・ホバー・キーボードフォーカスで文字を読めるようにします。
* PCヘッダーの配色と公開ページの保存内容は変更しません。

= 0.19.8 =
* PC/タブレットのみのデザインでも、スマホで空表示にならず既存レイアウトを継承します。
* 対象サイトのElementorブレークポイントを使用し、ネストした編集コンテナの表示設定を維持します。
* QAも通常の実CSSを使い、表示の強制補正で不具合を隠さない方式に対応します。

= 0.19.7 =
* Elementor 4系でもPC・タブレット・スマホの端末別コンテナを正しく切り替えます。
* 実ページQAへ同じレスポンシブCSSを適用し、保存前後の比較条件を一致させます。
